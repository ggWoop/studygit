...
def tokenize(split_chunk):
    def _tokenize(x):
        try:
            ...
            print(e, x)
            return None
    with open(f"./data/tmp_{os.getpid()}.csv", "w",encoding='utf-8-sig',newline='') as fw:
...
def write_tokenized_data(data):
    with open("./data/baseball_tokenized.csv", "w",encoding='utf-8-sig',newline='') as fw:
        writer = csv.writer(fw)
        writer.writerow(["url", "datetime_str", "title", "content", "title_tokens", "content_tokens"])
        for row in data:
            writer.writerow(row)
...
if __name__ == '__main__':
    ...
    tokenized_data = process_map(tokenize, split_list, max_workers=6)
